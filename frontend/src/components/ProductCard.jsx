...
import { Heart, BadgeCheck, Sparkles, Star, MapPin } from 'lucide-react';
        ...
        <motion.div whileHover={{ y: -2 }} className="bento-card flex gap-4 p-4">
          ...
          <div className="flex flex-1 flex-col">
            ...
            <div className="mt-1 flex items-center gap-2 text-xs text-muted-foreground">
              <MapPin className="h-3 w-3" /> {product.location} · {product.postedDays}d ago
            </div>
            <div className="mt-2 flex items-center gap-2">
              {product.verifiedBadge && <span className="inline-flex items-center gap-1 rounded-full bg-teal-50 px-2 py-0.5 text-[10px] font-semibold text-teal-700 dark:bg-teal-500/10 dark:text-teal-400"><BadgeCheck className="h-3 w-3" />Verified</span>}
              <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><Star className="h-3 w-3 fill-amber-400 text-amber-400" />{product.seller.rating}</span>
            </div>
      ...
      <motion.div whileHover={{ y: -4 }} transition={{ type: 'spring', stiffness: 220, damping: 20 }} className="bento-card overflow-hidden">
        <div className="relative aspect-square overflow-hidden bg-muted">
          ...
          {product.verifiedBadge && (
            <span className="absolute right-3 top-3 inline-flex items-center gap-1 rounded-full bg-teal-500 px-2 py-1 text-[10px] font-bold text-white shadow-lg">
        ...
        <div className="p-4">
          ...
          <div className="mt-3 flex items-end justify-between">
            <div>
              <p className="font-heading text-lg font-bold">${product.price.toLocaleString()}</p>
